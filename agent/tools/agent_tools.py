    ...
    if not external_data:
       ...
       with open(external_data_path,"r",encoding="utf-8") as f:
            for line in f.readlines()[1:]:
                arr:list[str] = line.strip().split(",")  #去除前后空格
...
@tool(description="从外部系统中获取用户的使用记录.以纯字符串返回,如果未检索到返回空字符串")
def fetch_external_data(user_id:str,month:str)->str:
    ...
    try:
        ...
        logger.warning(f"[fetch_external-data]未能检索到用户:{user_id}在{month}的使用记录数据")
        return ""
...
if __name__ == '__main__':
    print(fetch_external_data.invoke({"user_id":"1001","month":month_arr[0]}))